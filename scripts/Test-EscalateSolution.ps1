...
$sourcePlan = GetOrCreate 'msdyn_projects' 'msdyn_projectid' "msdyn_subject eq '1:1s with my team'" @{ msdyn_subject = '1:1s with my team' }
$targetPlan = GetOrCreate 'msdyn_projects' 'msdyn_projectid' "msdyn_subject eq '1:1 with my manager'" @{ msdyn_subject = '1:1 with my manager' }
...
$cards = @('Priya - career development plan', 'Tom - blocked on vendor access', 'Sam - workload concerns')
$cardIds = @{}
foreach ($c in $cards) {
...
Write-Host "CARD_TOM=$($cardIds['Tom - blocked on vendor access'])"
Write-Host "CARD_SAM=$($cardIds['Sam - workload concerns'])"

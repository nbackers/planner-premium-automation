<#
  ...
  Populates the mock Planner Premium tables with a realistic 1:1 scenario:

    Plan  "1:1s with my team"        (SOURCE)
      Bucket "To discuss"
      Bucket "Escalate"              <- dragging a card here triggers the flow
      Bucket "Done"
    Plan  "1:1 with my manager"      (TARGET)
...
$sourcePlanId = Get-OrCreate -EntitySet $planSet -IdField "${Prefix}_mockplanid" `
    -Filter "$subject eq '1:1s with my team'" -Body @{ $subject = '1:1s with my team' }
Write-Host "  SOURCE_PLAN_ID = $sourcePlanId"

$targetPlanId = Get-OrCreate -EntitySet $planSet -IdField "${Prefix}_mockplanid" `
    -Filter "$subject eq '1:1 with my manager'" -Body @{ $subject = '1:1 with my manager' }
...
$cards = @(
    'Priya - career development plan',
    'Tom - blocked on vendor access',
    'Sam - workload concerns'
)
foreach ($c in $cards) {

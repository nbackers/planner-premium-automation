<#
  ...
  Builds a hand-over-ready solution for the Planner Premium "copy a card
  to another plan" automation.
...
if ($sol) { Write-Host '  exists' }
    ...
    Post 'solutions' @{
        uniquename = $SolutionName; friendlyname = 'Planner Premium - Copy card to another plan'
...
$envVars = [ordered]@{
    "${Prefix}_SourcePlanName"     = @{ d = 'Watch this plan'; h = 'The exact name of the plan cards are escalated from. Example: Team backlog' }
    "${Prefix}_EscalateBucketName" = @{ d = 'Escalate bucket'; h = 'The exact name of the bucket that triggers the copy. Example: Escalate' }
    "${Prefix}_TargetPlanName"     = @{ d = 'Copy into this plan'; h = 'The exact name of the plan copies should land in. Example: Escalations' }
    "${Prefix}_TargetBucketName"   = @{ d = 'Bucket for copies'; h = 'The exact name of the bucket in the target plan where copies land. Example: Escalated' }
}
...
$flowName = 'Copy card to another plan'
...
if ($wf) {
    ...
    $flowId = Post 'workflows' @{
        ...
        description = 'When a card is moved into the configured bucket, copy it into the target plan. Configured through environment variables.'
...
$createSteps = [ordered]@{
    'Create_operation_set'  = DvAction 'PerformUnboundAction' ([ordered]@{
            ...
            item = [ordered]@{ Description = 'Copied from source plan'; Project = $tgtPlanId } }) $null
                            ...
                            # loudly with an instruction rather than silently doing nothing.
                                    ...
                                    'Stop_and_explain' = [ordered]@{
                                        ...
                                        inputs   = @{ runStatus = 'Failed'; runError = @{
                                                ...
                                                message = "Could not find a plan called '$(PIn "${Prefix}_TargetPlanName")' with a bucket called '$(PIn "${Prefix}_TargetBucketName")'. Open Solutions > Planner Premium - Copy card to another plan and check the four name settings match your plans and buckets exactly, including capitals and spacing."
                                            } }
                                    } } }
                        }
                    }

# Worked example: copy a card to another plan

> **This is one example of a Planner Premium automation, not the point of the repo.**
> It exists to prove the patterns end to end against something concrete.
> For the reusable mechanics see [automation-patterns.md](automation-patterns.md),
> and for the schema see [table-reference.md](table-reference.md).

The scenario: when a card is dragged into a bucket named `Escalate`, a copy appears in a second
plan. It exercises the trigger, condition, dedupe guard, environment-variable configuration and
Schedule API create in one flow.
...
| Setting | Example |
| --- | --- |
| Watch this plan | `Team backlog` |
| Escalate bucket | `Escalate` |
| Copy into this plan | `Escalations` |
...
GET https://orgXXXX.crm.dynamics.com/api/data/v9.2/msdyn_projects
      ...
      &$filter=contains(msdyn_subject,'Backlog')
```

You want:
- `SOURCE_PLAN_ID` - the plan cards are escalated from
- `TARGET_PLAN_ID` - the plan copies land in
...
created. Without it you would have 9 duplicates in the target plan.
...
4. Check the card appears in the target plan in the Planner UI.
...
| `Test-ExampleSolution.ps1` | Seed plans/buckets/cards for an end-to-end test |
| `Build-MockPlannerSchema.ps1` | Create mock tables mirroring the schema, for environments without Project |
| `Seed-MockPlannerData.ps1` | Seed the mock two-plan scenario |
